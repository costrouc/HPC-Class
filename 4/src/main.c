...
int main(int argc, char *argv[])
{
  ...
  for ( i=0; i<num_tests; i++ )
      ...
      for ( j=0; j<NUM_REPEAT_TEST; j++ )
	{
	  ...
	  unsigned int i;
	  for ( i=0; i<m*n; i++ )
	    Q[i] = temp[i] = A[i] = (double) rand() / RAND_MAX;
	  ...
	  /* Regular QR implementation 
	  double *tau = (double *) malloc(sizeof(double) * m * n);
	  LAPACKE_dgeqrf( LAPACK_COL_MAJOR, m, n, Q, m, tau );
	  ...
	  LAPACKE_dorgqr( LAPACK_COL_MAJOR, m, n, n, Q, m, tau );
	  */
